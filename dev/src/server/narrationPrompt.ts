...
const SEGMENT_TONE: Record<NarrationInput['segment'], string> = {
  ...
  C: `Segment C (${SEGMENT_PROFILES.C.name}) is under real time pressure — goal: "${SEGMENT_PROFILES.C.goal}." Write the SHORTEST possible phrasing: one short sentence, no subordinate clauses, no explanation of reasoning, and no suggestion to pause, compare, or reconsider. State the dollar tax figure as the very first word or clause of your sentence — before naming any fund, before describing what was sold, before anything else; never lead with "Selling" or "Sold," which puts the transaction before the figure, exactly backwards. There is no single correct sentence shape for this: construct a fresh, natural-sounding way to lead with the figure each time rather than defaulting to a repeated template — two responses to two different transactions should not read as the same sentence with the numbers swapped. Two explicit content rules for this segment, not just a style preference — a shorter sentence that still contains this content is not short enough: (1) NEVER state the short-term and long-term gain/loss breakdown separately, even briefly — state only the single net tax figure for the transaction; (2) NEVER state per-asset-class allocation percentage shifts (stocks/bonds/reserves, before vs. after, or any "-X% stocks" style figure) — not even a qualitative mention of an allocation effect, even as a trailing clause that otherwise reads naturally (e.g. nothing like "...lowering international equity exposure" or "...shifting the mix toward bonds"). Stop the sentence immediately after stating the fund(s) and dollar amount(s) sold — do not add a trailing clause explaining what the sale accomplishes, means, or affects, of any kind; a natural-sounding sentence that ends there is not incomplete, it is the goal. Both are tax detail and optional extra work in their own right, exactly what this reader's own research says the tool must avoid, regardless of how tersely they are phrased. What this reader needs the tool to avoid: ${SEGMENT_PROFILES.C.toolMustAvoid}.`,
...
export function buildNarrationPrompt(input: NarrationInput): { system: string; user: string } {
  const system = [
    ...
    '"effective_rate", if present, is already expressed as a percentage number — state it with a "%" sign using that value directly (e.g. "0.26" means "0.26%", not "26%"; "2.12" means "2.12%"). Never multiply or divide it by 100 again before stating it.',
