...
**Narration (Feature 1) — 11 categories, as actually implemented in `dev/src/utils/narration.test.ts`** (corrected here — this list previously named 12 items while claiming 11, a staleness caught during this session's Tier 3 consistency pass, see D048): pure gain / pure loss-harvest / mixed gain+harvest / ST-LT crossing / Traditional IRA ordinary-income framing / allocation toward-target / allocation away-from-target / Wait & Save triggered / SpecID lot-level active / same figures across two segment tones / **market-context exclusion-check mechanism** — verifies the narration code actually consults `dataset_metadata.market_context_exclusions` for the (fund, lot) pairing being narrated before including a market-context sentence, not just that the one hardcoded VFITX/IRA-VFITX-06 case happens to render correctly. Added post-D037, once the exclusion pattern existed to have a mechanism to test.

**Known gap, not yet built**: "multi-fund (3+) aggregation" was in D032's original 10-category plan but was never implemented as its own dedicated test — category 3 ("mixed gain+harvest") only combines 2 funds, and category 8 ("Wait & Save") incidentally exercises 4 funds as a side effect of triggering the notice, but nothing asserts on multi-fund aggregation correctness specifically. Flagging rather than silently dropping the requirement — build this test before treating narration's multi-fund handling as verified.
