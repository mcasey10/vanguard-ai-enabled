...
- **Prompt-quality rules** (`dev/src/server/narrationPrompt.ts`, see `DECISIONS.md` D045, D047): explicit dollar/share/percent formatting; the market-context figure must be framed as the fund's trailing-12-month return as of a fixed reference date, never as lot-specific ("since you bought it"); natural-language phrasing, never a field's internal name, and uninformative zero-figures omitted (except a $0 tax caused by IRA/Roth account type, which is meaningful and must be stated); Traditional IRA lines must state what's actually taxed (the withdrawal, as ordinary income) rather than just negating the gain/loss shown; segment tone is structured by rule, not just length — segment A scales to one short sentence *per fund* for multi-fund transactions, not one sentence total regardless of fund count.
...
- Provider adapters, each a concrete `NarrationGenerator`: `dev/src/server/generators/geminiGenerator.ts` (active default, `gemini-3.5-flash-lite` — `gemini-2.5-flash-lite` was retired for new API keys shortly after D042, see D043), `dev/src/server/generators/anthropicGenerator.ts` (working alternative, `claude-sonnet-5`)
